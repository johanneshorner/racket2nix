...
callPackage (
  ...
  let
    it-attrs = integration-tests.attrs;
  in
  let
    packagesFromFile =
      name: file:
      let
        buildInputs =
          let
            wordsToList =
              words: builtins.filter (s: (builtins.isString s) && s != "") (builtins.split "[ \n]+" words);
          in
          map buildRacketPackage (wordsToList (builtins.readFile file));
      in
      buildDrvs name buildInputs;
    attrs = rec {
      racket-doc = buildRacketPackage "racket-doc";
      typed-map-lib-generated = buildRacket {
        package = "typed-map-lib";
        buildNix = true;
      };
      ...
      light-tests = buildDrvs "light-tests" (
        [
          typed-map-lib
          typed-map-lib-generated
          typed-map-lib-generated.flat
        ]
        ++ builtins.attrValues integration-tests
      );
      heavy-tests = buildDrvs "heavy-tests" [
        racket-doc
        racket-doc.flat
      ];
