...
callPackage (
  ...
  let
    ...
    buildRacketFlat =
      { ... }@args:
      (pkgs.extend (self: super: { racket2nix = super.racket2nix-stage0.flat; })).buildRacket (
        args // { flat = true; }
      );

    buildThin =
      { ... }@args:
      (pkgs.extend (self: super: { racket2nix = super.racket2nix-stage0.thin; })).buildThinRacket args;
    ...
    stage1 =
      buildRacket {
        ...
        inherit attrOverrides;
      }
      // {
        flat = buildRacketFlat {
          package = ./nix;
